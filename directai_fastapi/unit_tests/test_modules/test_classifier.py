...
class TestImageClassifier(unittest.TestCase):
    ...
    image_classifier = NotImplemented  # type: ClassVar[ZeroShotImageClassifierWithFeedback]
    ...
    def test_random_tensor_classification_in_batch(self) -> None:
        random_tensors = torch.rand(128, 3, *self.image_classifier.img_size, device=self.image_classifier.device)
        bs_1_raw_scores = []
        with torch.no_grad():
            for random_tensor in random_tensors:
                ...
                bs_1_raw_scores.append(raw_scores)
            
            bs_1_raw_scores = torch.cat(bs_1_raw_scores, dim=0)
