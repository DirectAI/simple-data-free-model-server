...
from modeling.tensor_utils import batch_encode_cache_missed_list_elements, image_bytes_to_tensor
...
class ZeroShotImageClassifierWithFeedback(nn.Module):
    def __init__(self,
        ...
        self.augmented_label_encoding_cache = LRU(lru_cache_size) if lru_cache_size > 0 else None
        self.not_augmented_label_encoding_cache = LRU(lru_cache_size) if lru_cache_size > 0 else None
        
        preprocess_config = self.model.visual.preprocess_cfg
        self.img_mean = torch.tensor(preprocess_config['mean']).view(1, 3, 1, 1).to(self.device)
        self.img_std = torch.tensor(preprocess_config['std']).view(1, 3, 1, 1).to(self.device)
    ...
    def encode_image(self, image: torch.Tensor|bytes) -> torch.Tensor:
        ...
        for i in range(0, image.size(0), self.max_image_batch_size):
            features_subset = self.model.encode_image(image[i:i+self.max_image_batch_size])
    ...
    def _encode_text(self, text: list[str], augment: bool = True) -> torch.Tensor:
        ...
        for i in range(0, len(tokenized), self.max_text_batch_size):
            features_subset = self.model.encode_text(tokenized[i:i+self.max_text_batch_size])
    ...
    def encode_text(self, text: list[str], augment: bool = True) -> torch.Tensor:
        if augment:
            return batch_encode_cache_missed_list_elements(
                partial(self._encode_text, augment=True), text, self.augmented_label_encoding_cache
            )
        else:
            return batch_encode_cache_missed_list_elements(
                partial(self._encode_text, augment=False), text, self.not_augmented_label_encoding_cache
            )
    ...
    def forward(
        ...
        pos_labels_to_master_inds, pos_labels_list = zip(*[v for label, incs in inc_sub_labels_dict.items() for v in zip([label_to_ind[label]]*len(incs), incs)])
        ...
        offsets = torch.arange(num_images).unsqueeze(1).expand(-1, num_incs).flatten()*num_labels
        offsets = offsets.to(self.device)
        indices_for_max = torch.tensor(pos_labels_to_master_inds).to(self.device).repeat(num_images) + offsets
        
        max_pos_scores_flat, _ = scatter_max(pos_scores.view(-1), indices_for_max, dim_size=num_images*num_labels)
        ...
        if len(exc_sub_labels_dict) > 0:
            neg_labels_to_master_inds, neg_labels_list = zip(*[v for label, excs in exc_sub_labels_dict.items() for v in zip([label_to_ind[label]]*len(excs), excs)])
            ...
            offsets = torch.arange(num_images).unsqueeze(1).expand(-1, num_excs).flatten()*num_labels
            offsets = offsets.to(self.device)
            indices_for_max = torch.tensor(neg_labels_to_master_inds).to(self.device).repeat(num_images) + offsets
            
            max_neg_scores_flat, _ = scatter_max(neg_scores.view(-1), indices_for_max, dim_size=num_images*num_labels)
            max_neg_scores = max_neg_scores_flat.view(num_images, num_labels)
            
            raw_scores = torch.where(max_pos_scores > max_neg_scores, max_pos_scores, 1 - max_neg_scores)
        else:
