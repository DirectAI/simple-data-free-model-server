...
def flash_attn_owl_vit_encoder_forward(
    ...
    ), "output_attentions not supported for flash attention implementation"
    assert (
        attention_mask is None
    ), "attention_mask not supported for flash attention implementation"
    ...
    ), "causal_attention_mask not supported for flash attention implementation"
    ...
    query_states = query_states.contiguous().view(
        bsz, tgt_len, self.num_heads, self.head_dim
    )
    key_states = key_states.contiguous().view(
        bsz, tgt_len, self.num_heads, self.head_dim
    )
    value_states = value_states.contiguous().view(
        bsz, tgt_len, self.num_heads, self.head_dim
    )
    ...
    def __init__(self, vision_model: Owlv2VisionModel) -> None:
        ...
        for owlv2_vision_model_encoder_layer in self.vision_model.encoder.layers:
            owlv2_vision_model_encoder_layer.self_attn.forward = partial(
                ...
                owlv2_vision_model_encoder_layer.self_attn,
            )
...
class WrappedImageEmbedder(nn.Module):
    ...
    def forward(
        ...
        image_class_embeds = image_class_embeds / image_class_embeds.norm(
        ...
        image_class_embeds = image_class_embeds.gather(
            1, indices.unsqueeze(-1).expand(-1, -1, image_class_embeds.shape[-1])
        )
        logit_shift = logit_shift.gather(
            1, indices.unsqueeze(-1).expand(-1, -1, logit_shift.shape[-1])
        )
        logit_scale = logit_scale.gather(
            1, indices.unsqueeze(-1).expand(-1, -1, logit_scale.shape[-1])
        )
        pred_boxes = pred_boxes.gather(
            1, indices.unsqueeze(-1).expand(-1, -1, pred_boxes.shape[-1])
        )
...
class ZeroShotObjectDetectorWithFeedback(nn.Module):
    def __init__(
        ...
        self.device = device
        self.model = (
            Owlv2ForObjectDetection.from_pretrained(model_name).to(device).eval()
        )
        ...
        self.rgb_means = torch.tensor([0.485, 0.456, 0.406], device=device).view(
            1, 3, 1, 1
        )
        self.rgb_stds = torch.tensor([0.229, 0.224, 0.225], device=device).view(
            1, 3, 1, 1
        )
    ...
    def forward(
        ...
        if isinstance(image, bytes):
            assert (
                image_scale_ratios is None
            ), "image_scale_ratios must be None if image is bytes as we define the scale internally"
            image_tensor, image_scale_ratios = created_padded_tensor_from_bytes(
                image, self.image_size
            )
        else:
            assert (
                image_scale_ratios is not None
            ), "image_scale_ratios must be provided if image is a tensor as we cannot derive the scale internally"
            image_tensor = image
        ...
        max_pos_scores = max_pos_scores.view(
            image_data["pred_boxes"].shape[0],
            image_data["pred_boxes"].shape[1],
            num_labels,
        )
        max_neg_scores = max_neg_scores.view(
            image_data["pred_boxes"].shape[0],
            image_data["pred_boxes"].shape[1],
            num_labels,
        )
        ...
        batched_predicted_boxes = batched_run_nms_based_box_suppression_for_all_objects(
            ...
            torch.tensor(
                [label_conf_thres.get(label, 0.0) for label in labels],
                device=self.device,
            ),
            nms_thre,
...
@torch.jit.script
def find_in_sorted_tensor(
    sorted_tensor: torch.Tensor, query: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor]:
...
@torch.jit.script
def run_nms_via_adjacency_list(
    ...
    # NOTE: we've already computed the graph so we don't need to supply an NMS threshold
    first_node = adjacency_list[0]
    zero_tensor = torch.tensor([0], device=first_node.device)
    change_inds = (first_node[1:] != first_node[:-1]).nonzero()[:, 0]
    len_tensor = torch.tensor([first_node.shape[0]], device=first_node.device)
    inds_of_adj_boxes = torch.cat([zero_tensor, change_inds + 1, len_tensor])

    # we then run a NMS over the boxes
    # need to keep track of which boxes have survived, which means an index per possible box
    # note that some boxes may have already been filtered out
    # so valid_box_indices_by_descending_score may not contain all indices
    # TODO: switch this to a sparse tensor for efficiency
    highest_possible_ind = max(
        valid_box_indices_by_descending_score.max().item(),
        adjacency_list[0].max().item(),
        adjacency_list[1].max().item(),
    )
    assert isinstance(highest_possible_ind, int)  # this is just to make mypy happy
    has_survived = torch.zeros(
        highest_possible_ind + 1,
        device=valid_box_indices_by_descending_score.device,
        dtype=torch.bool,
    )
    has_survived[valid_box_indices_by_descending_score] = 1

    # check which boxes have graph connections
    unique_nodes = first_node[inds_of_adj_boxes[:-1]]
    has_connection, graph_node_indices = find_in_sorted_tensor(
        unique_nodes, valid_box_indices_by_descending_score
    )
    connected_sorted_pro_valid_inds = valid_box_indices_by_descending_score[
        has_connection
    ]
    graph_indices = graph_node_indices[has_connection]

    # supress the boxes for which their (unsupressed) neighbors have higher scores
    for i, j in zip(connected_sorted_pro_valid_inds, graph_indices):
        ...
        adj_boxes = adjacency_list[1][remapped_start_ind:remapped_end_ind]
        has_survived[adj_boxes] = 0

    survive_inds = valid_box_indices_by_descending_score[
        has_survived[valid_box_indices_by_descending_score]
    ]
...
@torch.jit.script
def compute_candidate_nms_via_adjacency_list(
    ...
    adjacent_con_max = scatter_max(
        expanded_con_max, adjacency_list[1], dim_size=con_max.shape[0]
    )[0]
    ...
    pro_valid = (
        (pro_max >= conf_thre) * (pro_max >= adjacent_con_max) * (pro_max >= con_max)
    )
...
def run_nms_based_box_suppression_for_one_object(
    ...
    survive_inds = compute_candidate_nms_via_adjacency_list(
...
def run_nms_based_box_suppression_for_all_objects(
    ...
    futures = [
        torch.jit.fork(
            ...
            image_scale,
            conf_thres[i],
            run_nms=not run_class_agnostic_nms,
        )
        for i in range(pro_max.shape[1])
    ]

    # there appears to be a bug in JIT related to star expansion that stops us from just using the following list comprehension:
    # object_boxes, box_indices = zip(*[torch.jit.wait(fut) for fut in futures])
    object_boxes: list[torch.Tensor] = []
    box_indices: list[torch.Tensor] = []
    for fut in futures:
        boxes_with_scores, survive_inds = torch.jit.wait(fut)
        object_boxes.append(boxes_with_scores)
        box_indices.append(survive_inds)

    if run_class_agnostic_nms:
        # first take the top class prediction for each box
        # we will of course use a scatter_max for this
        all_object_boxes = torch.cat(object_boxes, dim=0)
        all_box_indices = torch.cat(box_indices, dim=0)
        all_box_class_assignments = torch.cat(
            [
                torch.full((box.shape[0],), i, dtype=torch.long, device=box.device)
                for i, box in enumerate(object_boxes)
            ],
            dim=0,
        )
        all_object_confidences = all_object_boxes[:, 4]

        # we use an out argument here so we can control the default value
        box_max_confidences = torch.zeros(pred_boxes.shape[0], device=pred_boxes.device)
        _, max_confidence_indices = scatter_max(
            all_object_confidences, all_box_indices, out=box_max_confidences
        )

        survived_inds = box_max_confidences.nonzero().squeeze(1)

        # now filter the survived inds via NMS
        sorted_survived_inds = survived_inds[
            box_max_confidences[survived_inds].argsort(descending=True)
        ]
        post_nms_survived_inds = run_nms_via_adjacency_list(
            sorted_survived_inds, adjacency_list
        )

        # and accumulate the boxes
        object_survived_inds = max_confidence_indices[post_nms_survived_inds]
        survived_class_assignments = all_box_class_assignments[object_survived_inds]
        survived_object_boxes = all_object_boxes[object_survived_inds]

        # loop through based on class_assignments to create list of per-class boxes
        # TODO: this could be done more efficiently
        object_boxes = [
            survived_object_boxes[survived_class_assignments == i]
            for i in range(pro_max.shape[1])
        ]
