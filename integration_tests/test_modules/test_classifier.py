...
class TestClassifyDeploy(unittest.TestCase):
    ...
    def test_deploy_classifier_failure_include_missing(self) -> None:
        body = {
            "classifier_configs": [
                {"name": "bottle", "examples_to_exclude": []},
                {
                    "name": "can",
    ...
    def test_deploy_classifier_success_without_exclude(self) -> None:
        body = {
            "classifier_configs": [
                {"name": "bottle", "examples_to_include": ["bottle"]},
                {"name": "can", "examples_to_include": ["can"]},
            ]
        }
        response = requests.post(self.endpoint + "deploy_classifier", json=body)
        response_json = response.json()
...
class TestClassifyInference(unittest.TestCase):
    ...
    def test_classify(self) -> None:
        ...
        expected_response = {
            "scores": {"bottle": 0.6345123648643494, "can": 0.36548763513565063},
            "pred": "bottle",
            "raw_scores": {"bottle": 0.6409257650375366, "can": 0.6023120284080505},
        }
        response = requests.post(self.endpoint + "classify", params=params, files=files)
    ...
    def test_deploy_and_classify(self) -> None:
        ...
        classify_response = requests.post(
            self.endpoint + "classify", params=params, files=files
        )
        ...
        self.assertLess(
            compute_kl_divergence_based_classification_loss(
                expected_response, classify_response_json
            ),
            2e-5,
        )
    ...
    def test_deploy_and_classify_without_augment_examples(self) -> None:
        ...
        params = {"deployed_id": deployed_id}
        expected_response = {
            "scores": {"bottle": 0.5853425860404968, "can": 0.4146574139595032},
            "pred": "bottle",
            "raw_scores": {"bottle": 0.6374971270561218, "can": 0.6133649945259094},
        }
        classify_response = requests.post(
            self.endpoint + "classify", params=params, files=files
        )
        ...
        # Starting Classify Call
        params = {"deployed_id": deployed_id_with_augmented_examples}
