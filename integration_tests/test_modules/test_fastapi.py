...
def compute_naive_bipartite_detection_loss(detections_set_1: List[SingleDetectionResponse], detections_set_2: List[SingleDetectionResponse], unmatched_loss: float =1.0) -> float:
    ...
    total_loss += unmatched_loss * (max(num_detections_1, num_detections_2) - len(row_ind))  # Add loss for unmatched detections

    # Compute the average loss
    average_loss = total_loss / max(num_detections_1, num_detections_2) # Normalizing by the larger set size
    return average_loss
...
class TestDetect(unittest.TestCase):
    ...
    def test_deploy_and_detect(self) -> None:
        ...
        sample_fp = "sample_data/coke_through_the_ages.jpeg"
        expected_detect_response_unaccelerated = [[{'tlbr': [167.0, 151.0, 267.0, 494.0], 'score': 0.473, 'class': 'bottle'}, {'tlbr': [407.0, 152.0, 510.0, 495.0], 'score': 0.466, 'class': 'bottle'}, {'tlbr': [534.0, 150.0, 632.0, 494.0], 'score': 0.457, 'class': 'bottle'}, {'tlbr': [39.3, 183.0, 141.0, 494.0], 'score': 0.45, 'class': 'bottle'}, {'tlbr': [653.0, 151.0, 756.0, 497.0], 'score': 0.45, 'class': 'bottle'}, {'tlbr': [290.0, 160.0, 388.0, 493.0], 'score': 0.443, 'class': 'bottle'}, {'tlbr': [780.0, 158.0, 879.0, 498.0], 'score': 0.418, 'class': 'bottle'}, {'tlbr': [910.0, 108.0, 1030.0, 501.0], 'score': 0.385, 'class': 'bottle'}, {'tlbr': [1070.0, 170.0, 1160.0, 498.0], 'score': 0.364, 'class': 'bottle'}]]
        with open(sample_fp, 'rb') as f:
            file_data = f.read()
        files = {
            'data': (sample_fp, file_data, "image/jpg"),
    ...
    def test_deploy_with_long_prompt_and_detect(self) -> None:
        ...
        # Starting Detect Call
        sample_fp = "sample_data/coke_through_the_ages.jpeg"
        expected_detect_response_unaccelerated = [[{'tlbr': [2.04, -1.39, 1190.0, 636.0], 'score': 0.0195, 'class': 'sample_prompt'}]]
        with open(sample_fp, 'rb') as f:
            file_data = f.read()
        files = {
            'data': (sample_fp, file_data, "image/jpg"),
    ...
    def test_deploy_and_detect_without_augmented_examples(self) -> None:
        ...
        # Starting Detect Call
        params = {
            'deployed_id': deployed_id_augmented_examples
        }
        ...
        detect_response_augmented_examples_json = detect_response_augmented_examples.json()
    ...
    def test_deploy_with_and_without_class_agnostic_nms(self) -> None:
        ...
        self.assertEqual(
            len(detect_response_json[0]),
            1
        )
        detected_classes = set([detection['class'] for detection in detect_response_json[0]])
        self.assertEqual(
            detected_classes,
            {'head'}
        )
        ...
        # Starting Detect Call
        params = {
            'deployed_id': deployed_id_class_based_nms
        }
        ...
        self.assertEqual(
            len(detect_response_class_based_nms_json), 1
        )
        self.assertEqual(
            len(detect_response_class_based_nms_json[0]),
            2
        )
        detected_classes = set([detection['class'] for detection in detect_response_class_based_nms_json[0]])
        self.assertEqual(
            detected_classes,
            {'head', 'face'}
        )
