...
build_app() {
    ...
    gradio_output=$(docker run --rm simple-data-free-model-server-local_gradio:latest mypy . --disallow-untyped-defs --disallow-incomplete-defs)
    echo "$gradio_output"
...
build_testing() {
    ...
    integration_testing_output=$(docker run --rm simple-data-free-model-server-integration_tests:latest mypy integration_tests/test.py test_modules --disallow-untyped-defs --disallow-incomplete-defs)
